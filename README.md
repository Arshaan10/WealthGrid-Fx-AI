...
Premium Forex AI grid desk — gold and black, Phase 1.

Marketing site, member dashboard, and admin dashboard on **Next.js App Router**, **TypeScript**, **Tailwind CSS**, **Prisma + SQLite**, and **NextAuth** (email/password). Deposits and withdrawals are **DEX placeholders**: they record pending intents and never call a chain.

Brand spelling is **Whealth**, not Wealth.

Package, referral, rank, and gift numbers live in [`config/rewards.ts`](config/rewards.ts).

## What Phase 1 includes

- Public pages: Home, About, Packages, Rewards, Ranks, How it works, FAQ, Contact, Login, Register
- Member desk (`/dashboard`): wallets + ledger, activate Pro, deposit/withdraw stubs, referrals, rewards, profile
- Admin desk (`/admin`): users, packages/activations, DEX queue approve/reject, reward config view, ranks, announcements CRUD, audit log
- Lifetime business data in Prisma (users, wallets, ledger, packages, referrals, payouts, ranks, intents, announcements, audit)
...
Demo ships with an active **Pro** package, sample ledger, a pending deposit, and a reserved withdrawal so the admin queue is exercisable.
...
Member **Deposit** and **Withdraw** screens show **Connect wallet (coming next)**. Confirming writes `DepositIntent` or `WithdrawalRequest` rows. Withdrawals reserve available balance. Admins approve or reject on `/admin/queue`.
...
config/rewards.ts    # single source for package/rank numbers
